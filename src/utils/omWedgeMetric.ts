/**
 * 추가 차단영역(쐐기) 소실율 산출 — Rust az×elev 시간 히스토그램을 건물별 컷오프로 슬라이스.
 *
 * 헤드라인 심각도 지표: "분석 대상 건물이 새로 가리는(쐐기) 노출 비행시간 중 소실된 비율"
 * = 노출 조건부 소실율. 방위 비교(교란·반사실 부재)를 대체하는 인과 지표.
 *
 * 쐐기 밴드 = angleWithout(az) ≤ elev < angleWith(az) — classifyObstacleLosses 의 buildingCaused
 ...
 *
 *   분모(노출) = Σ(쐐기 셀의 추적시간 + 소실시간),  분자 = Σ(쐐기 셀의 소실시간)
 *   소실율(%) = 분자 / 분모 × 100
 *
...
import type { AzSector, AzElevCell, WedgeMetricResult, WedgeDaySeries } from "../types/obstacle";
import { makePanoramaSampler } from "./obstacleAnalysisHelpers";
import { gradeWedge, weightedTrendSlope } from "./omStats";
 ...
 * @param panoWith         지형+기존지물+분석대상 파노라마 (해당 레이더). 없으면 쐐기 0 → 노출 없음.
 ...
 */
export function computeWedgeMetric(
  histogramsByDay: WedgeDayHist[],
  ...
  //   panoWith 없으면 차단각 0(쐐기 없음 → 노출 0).
  //   panoWithout 없으면(제외대상 0) sWo=sW → without==with → 쐐기 0(과대귀속 방지).
  //   ※ terrain 폴백 금지: 비대상 GIS 건물 차단까지 쐐기에 포함돼 과대귀속됨.
  ...
  for (const dh of histogramsByDay) {
    ...
    for (const c of dh.cells) {
      ...
      const elC = HIST_ELEV_MIN_DEG + c.elev_bin * HIST_ELEV_BIN_DEG + HIST_ELEV_BIN_DEG / 2;
      // 쐐기 밴드: without(지형) ≤ elev < with(지형+대상) — 대상 건물이 새로 가리는 띠
      if (elC >= angleWithoutAt(azC) && elC < angleWithAt(azC)) {
        dayLoss += c.loss_time_s;
        dayExposure += c.track_time_s + c.loss_time_s;
      }
    }
  ...
  const wedgeLossRatePct = totalExposure > 0 ? (totalLoss / totalExposure) * 100 : 0;
  ...
  const g = gradeWedge(wedgeLossRatePct, dayCount, totalExposure);
