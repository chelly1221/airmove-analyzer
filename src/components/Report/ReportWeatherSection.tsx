...
export default function ReportWeatherSection({ sectionNum, weather }: Props) {
  ...
  return (
    <div className="space-y-5">
      <h2 className="text-[19px] font-bold text-gray-900 border-b-2 border-gray-800 pb-1">
        {sectionNum}. 기상 조건 분석
      </h2>
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="rounded border border-gray-200 p-2.5 text-center">
          <div className="text-[12px] text-gray-400">평균 운량</div>
          <div className="text-[16px] font-bold text-gray-800">{avgCloud.toFixed(0)}%</div>
        </div>
        <div className="rounded border border-gray-200 p-2.5 text-center">
          <div className="text-[12px] text-gray-400">평균 시정</div>
          <div className="text-[16px] font-bold text-gray-800">{(avgVis / 1000).toFixed(1)}km</div>
        </div>
        <div className="rounded border border-gray-200 p-2.5 text-center">
          <div className="text-[12px] text-gray-400">평균 풍속</div>
          <div className="text-[16px] font-bold text-gray-800">{avgWind.toFixed(1)}m/s</div>
        </div>
        <div className="rounded border border-gray-200 p-2.5 text-center">
          <div className="text-[12px] text-gray-400">평균 기압</div>
          <div className="text-[16px] font-bold text-gray-800">{avgPressure.toFixed(0)}hPa</div>
        </div>
      </div>

      {/* 시간별 기상 테이블 */}
      <div>
        <h3 className="text-[14px] font-semibold text-gray-700 mb-1.5">시간별 기상 현황</h3>
        ...
        {hourlyData.length > 48 && (
          <p className="mt-1 text-[12px] text-gray-400">* 48시간 초과 데이터는 생략됨</p>
        )}
      </div>
      ...
      {ductingHours.length > 0 && (
        <div className="rounded border border-yellow-300 bg-yellow-50 p-3">
          <h3 className="text-[14px] font-semibold text-yellow-800 mb-1">덕팅 가능 시간대 ({ductingHours.length}시간)</h3>
          ...
          <div className="mt-1.5 text-[12px] text-yellow-600">
