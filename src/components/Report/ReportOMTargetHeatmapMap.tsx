 ...
 *
 *  - 결합: 모든 레이더의 track_heatmap(남한 전역 bbox 전방위 전수 밀도, 표시 전용) 을 하나의 강도 버퍼에 합산.
 ...
 *  - 원 밖 표적: Rust 그리드가 남한 전역 bbox 전수라 60NM 원 밖 표적도 표시 창 안이면 전부 보인다(구버전 ±150NM 캐시는 재분석 전까지 남부 미포함).
...
const R_KM = 6371;
/** 범위 원 = 60NM(OM 통계 스코프). 표시 유지 — 원 밖 표적도 남한 전역 그리드로 표시됨. */
