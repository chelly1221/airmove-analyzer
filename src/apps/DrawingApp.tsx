...
import { FolderOpen, Loader2, Plane, ChevronDown, Radar } from "lucide-react";
...
function useAssFilePicker() {
  ...
  const parseWithFilter = useCallback(async (filter: ParseFilterResult) => {
    ...
    if (outcome.failed) {
      console.error("[Drawing] 배치 파싱 실패");
      useToastStore.getState().addToast("파싱 실패 — 파일을 읽지 못했습니다");
    } else if (!outcome.complete) {
      console.warn("[Drawing] 파싱 수신 불완전 — 일부 청크가 유실됐을 수 있습니다");
    ...
    if (!consolidatingRef.current && useAppStore.getState().workerPointCount > 0) {
      ...
      try {
        ...
        if (!(e instanceof ConsolidateSuperseded)) console.error("[Drawing] 비행 통합 실패:", e);
      } finally {
...
export default function DrawingApp() {
  ...
  const modeSCounts = useMemo(() => {
    ...
    for (const f of flights) counts.set(f.mode_s, (counts.get(f.mode_s) ?? 0) + f.point_count);
    return counts;
  }, [flights]);

  const validModeS = useMemo(() => {
    const valid = new Set<string>();
    for (const [ms, cnt] of modeSCounts) { if (cnt >= 10) valid.add(ms); }
    return valid;
  }, [modeSCounts]);
        ...
        <div data-tauri-drag-region className="flex flex-1 h-full items-center pl-4 gap-2">
          ...
          <div className="relative pointer-events-auto" ref={dropdownRef}>
            ...
            >
              ...
              <span className="max-w-[120px] truncate font-medium">{getFilterLabel()}</span>
                ...
                <div className="max-h-56 overflow-y-auto py-1 px-1">
                  ...
                  {aircraft.filter((a) => a.active).map((a) => (
                    <button
                      ...
                      className={`w-full px-3 py-1.5 text-left text-xs rounded transition-colors ${selectedModeS === a.mode_s_code ? "bg-[#a60739] text-white" : "text-gray-600 hover:bg-gray-100"}`}
                    >
                      <div className="flex items-center gap-2">
                        <span>{a.name}</span>
                        <span className={`text-[10px] ${selectedModeS === a.mode_s_code ? "text-white/60" : "text-gray-400"}`}>{a.mode_s_code}</span>
                      </div>
                    </button>
                  ))}
                  ...
                  {modeSOptions.length > 0 && <div className="border-t border-gray-200 my-1 mx-2" />}
                  {modeSOptions.map((ms) => (
                    <button
                      key={ms}
